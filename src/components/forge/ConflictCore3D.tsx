...
function WarpField({ scoreA, scoreB, eventTrigger }: CoreProps) {
  ...
  return (
    <group>
      ...
      <pointLight position={[0, 0, 5]} color={dominantColor} intensity={speed > 1 ? 4 : 0} distance={50} />
    </group>
  )
}

export default function ConflictCore3D({ scoreA, scoreB, eventTrigger }: CoreProps) {
  return (
    <div className="fixed inset-0 z-0 pointer-events-none opacity-50 mix-blend-screen">
      <Canvas camera={{ position: [0, 0, 10], fov: 45 }}>
        <fog attach="fog" args={['#06060b', 8, 25]} />
